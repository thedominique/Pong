...
void main_loop(SDL_Renderer* renderer, UDPsocket* udpsock, IPaddress server_addr, bool is_server) {
	...
	for (;;) {
		...
		int numrecv = SDLNet_UDP_Recv(udpsock, &packet); // Ta emot en packet i en specifierad socket
		// packet måste innehålla tillräckligt med allokerad data för det inkommande packet data ska få plats.
		if (numrecv > 0) {
			// Kollar "is_server" om vi startade som server eller klient.
			if (is_server) {
				bool is_client_player1;
				if (address_equal(packet.address, client_addr[0])) {
					// packet.address indikerar source/dest address 
					// Jämför i client_addr[] om den klient har tidigare kopplats med servern tidigare
					is_client_player1 = true;
				}
				else if (address_equal(packet.address, client_addr[1])) {
					// Eftersom den nya adressen inte stämmer med i client_addr, är det inte player1.
				...
				// player struct "client_packet"
				// "omvandlar" packet.data till (client_packet_t) och kopierar till client_packet
				client_packet_t* client_packet = (client_packet_t*)packet.data;


				// Kollar om det är player1 eller player2
...
int main(int argc, const char * argv[]) {
	...
	if (argc > 1) {
		printf("I'm a client!\n");
		if (SDLNet_ResolveHost(&server_addr, "192.168.0.23", local_port) != 0) {
