...
SDLNet_ResolveHost(&ip, "130.229.156.68", 1234); // server ip adress
	...
	// left flipper pos
int* sendxpos = cstrect.x;
int* sendypos = cstrect.y;

//get right flipper pos

int* getxpos = bstrect.x;
int* getypos = bstrect.y;
...
int main(int argc, const char * argv[]) {
	...
	while (!quit) {
		...
		while (SDL_PollEvent(&e) >= 0)
		{
			...
			SDLNet_TCP_Send(client, ballx, sizeof(ballx);
			SDLNet_TCP_Send(client, bally, sizeof(bally);
